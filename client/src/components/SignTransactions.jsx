...
const SignTransactions = () => {
  ...
  const fetchSignableDonations = async () => {
    try {
      const response = await fetch('http://localhost:5001/api/admin/signable-donations', {
        headers: {
  ...
  const handleReject = async (donation) => {
    ...
    try {
      const response = await fetch(`http://localhost:5001/api/admin/donations/${donation.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleStatusChange = async (donation, newStatus) => {
    ...
    try {
      ...
      const updateResponse = await fetch('http://localhost:5001/api/donations/direct-update-hash', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
