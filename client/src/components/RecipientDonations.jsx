...
const RecipientDonations = () => {
  ...
  const fetchDonations = async () => {
    if (!token) return;
    try {
      const response = await fetch(buildApiUrl('/api/recipients/donations'), {
  ...
  const handleStatusChange = async (donationId, status) => {
    try {
      const response = await fetch(buildApiUrl(`/api/donations/${donationId}/status`), {
        ...
        body: JSON.stringify({ status: status, donation_type: donations.find(d => d.id === donationId)?.donation_type })
      });
  ...
  const handleViewReceipt = (receiptPath) => {
    ...
    window.open(buildApiUrl(`/uploads/receipts/${receiptPath}`), '_blank');
  };
              ...
              <div className="donation-summary">
                ...
                {donation.donation_type === 'money' && donation.receipt_path && (
                  <button 
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      handleViewReceipt(donation.receipt_path);
                    }}
                  >
                    View Receipt
                  </button>
                    ...
                    {selectedDonation.receipt_path && (
                      <div className="receipt-section">
                        ...
                        <button 
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            handleViewReceipt(selectedDonation.receipt_path);
                          }}
                        >
                          📄 View Receipt
                        </button>
